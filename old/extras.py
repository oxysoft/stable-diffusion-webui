...
import shared
from core.plugins import sd_models
from shared import opts
from core import plugins, devicelib
from gradio.ui import plaintext_to_html
import piexif
...
def run_extras(extras_mode, resize_mode, image, image_folder, gfpgan_visibility, codeformer_visibility, codeformer_weight, upscaling_resize, upscaling_resize_w, upscaling_resize_h, upscaling_crop, extras_upscaler_1, extras_upscaler_2, extras_upscaler_2_visibility):
    ...
    for image, image_name in zip(imageArr, imageNameArr):
        ...
        images.save_image(image, path=outpath, basename="", seed=None, prompt=None, extension=opts.samples_format, metadata=info, short_filename=True,
...
def run_modelmerger(primary_model_name, secondary_model_name, interp_method, interp_amount, save_as_half, custom_name):
    ...
    primary_model_info = sd_models.checkpoints[primary_model_name]
    secondary_model_info = sd_models.checkpoints[secondary_model_name]

    print(f"Loading {primary_model_info.dir}...")
    primary_model = torch.load(primary_model_info.dir, map_location='cpu')

    print(f"Loading {secondary_model_info.dir}...")
    secondary_model = torch.load(secondary_model_info.dir, map_location='cpu')
