...
from modules.stable_diffusion_auto1111.SDJob import store_latent
...
all_samplers = [
    *[
        ...
        for label, funcname, aliases, options in [
...
def create_sampler(id, model):
    for s in samplers:
        if id == s.name or id in s.aliases:
            sampler = s.constructor(model)
            modules.stable_diffusion_auto2222.sdpaths.config = s

            return sampler

    raise ValueError(f'Unknown sampler {id}')
...
def single_sample_to_image(sample):
    x_sample = modules.stable_diffusion_2222.processing.decode_first_stage(SDPlugin.sdmodel, sample.unsqueeze(0))[0]
...
class VanillaStableDiffusionSampler:
    ...
    def sample_img2img(self, p, x, noise, conditioning, unconditional_conditioning, steps=None, image_conditioning=None):
        ...
        try:
            self.sampler.make_schedule(ddim_num_steps=steps, ddim_eta=self.eta, ddim_discretize=self.ddim_discretize, verbose=False)
        except Exception:
            self.sampler.make_schedule(ddim_num_steps=steps + 1, ddim_eta=self.eta, ddim_discretize=self.ddim_discretize, verbose=False)

        x1 = self.sampler.stochastic_encode(x, torch.tensor([t_enc] * int(x.shape[0])).to(devices.device), noise=noise)
        ...
        samples = self.launch_sampling(steps, lambda: self.sampler.decode(x1, conditioning, t_enc, unconditional_guidance_scale=p.cfg, unconditional_conditioning=unconditional_conditioning))
    ...
    def sample(self, p, x, conditioning, unconditional_conditioning, steps=None, image_conditioning=None):
        ...
        # existing code fails with certain step counts, like 9
        try:
            samples_ddim = self.launch_sampling(steps, lambda: self.sampler.sample(S=steps, conditioning=conditioning, batch_size=int(x.shape[0]), shape=x[0].shape, verbose=False, unconditional_guidance_scale=p.cfg, unconditional_conditioning=unconditional_conditioning, x_T=x, eta=self.eta)[0])
        except Exception:
            samples_ddim = self.launch_sampling(steps, lambda: self.sampler.sample(S=steps + 1, conditioning=conditioning, batch_size=int(x.shape[0]), shape=x[0].shape, verbose=False, unconditional_guidance_scale=p.cfg, unconditional_conditioning=unconditional_conditioning, x_T=x, eta=self.eta)[0])
