...
import sd_samplers, SDPlugin
# from SDPlugin import opts, cmd_opts

LANCZOS = (Image.Resampling.LANCZOS if hasattr(Image, 'Resampling') else Image.LANCZOS)


def image_grid(imgs, batch_size=1, rows=3):
    #     if opts.n_rows > 0:
    #         rows = opts.n_rows
    #     elif opts.n_rows == 0:
    #         rows = batch_size
...
def resize_image(resize_mode, im, width, height):
    def resize(im, w, h):
        ...
        #
        # if scale > 1.0:
        #     upscalers = [x for x in SDPlugin.sd_upscalers if x.name == opts.upscaler_for_img2img]
...
#
#     x = x.replace("[model_hash]", getattr(p, "sd_model_hash", SDPlugin.sd_model.hash))
#     x = x.replace("[date]", datetime.date.today().isoformat())
#     x = x.replace("[datetime]", datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
#     x = x.replace("[job_timestamp]", getattr(p, "job_timestamp", SDPlugin.state.job_timestamp))
#
...
#             for style in SDPlugin.prompt_styles.get_style_prompts(p.styles):
#                 if len(style) > 0:
