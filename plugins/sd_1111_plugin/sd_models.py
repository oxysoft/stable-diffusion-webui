...
def discover_models():
    all_infos.clear()
    all_paths = modellib.discover_models(model_dir=model_path, command_path=modules.stable_diffusion_auto2222.sdpaths.ckpt_dir, ext_filter=[".ckpt"])

    def modeltitle(path, shorthash):
        abspath = os.path.abspath(path)

        if modules.stable_diffusion_auto2222.sdpaths.ckpt_dir is not None and abspath.startswith(modules.stable_diffusion_auto2222.sdpaths.ckpt_dir):
            name = abspath.replace(modules.stable_diffusion_auto2222.sdpaths.ckpt_dir, '')
    ...
    cmd_ckpt = modules.stable_diffusion_auto2222.sdpaths.ckpt
    if os.path.exists(cmd_ckpt):
        h = get_model_hash(cmd_ckpt)
        title, short_model_name = modeltitle(cmd_ckpt, h)
        all_infos[title] = CheckpointInfo(cmd_ckpt, title, h, short_model_name, modules.stable_diffusion_auto2222.sdpaths.config)
        modules.stable_diffusion_auto2222.options.opts.data['sd_model_checkpoint'] = title
    ...
    for filename in all_paths:
        ...
        if not os.path.exists(config):
            config = modules.stable_diffusion_auto2222.sdpaths.config
...
def select_checkpoint(path=None):
    ...
    if len(all_infos) == 0:
        ...
        if modules.stable_diffusion_auto2222.sdpaths.ckpt is not None:
            print(f" - file {os.path.abspath(modules.stable_diffusion_auto2222.sdpaths.ckpt)}", file=sys.stderr)
        print(f" - directory {model_path}", file=sys.stderr)
        if modules.stable_diffusion_auto2222.sdpaths.ckpt_dir is not None:
            print(f" - directory {os.path.abspath(modules.stable_diffusion_auto2222.sdpaths.ckpt_dir)}", file=sys.stderr)
...
def load_model_weights(model, info):
    ...
    if info not in all_loaded:
        ...
        if not os.path.exists(vae_file) and modules.stable_diffusion_auto2222.sdpaths.vae_path is not None:
            vae_file = modules.stable_diffusion_auto2222.sdpaths.vae_path
        ...
        while len(all_loaded) > modules.stable_diffusion_auto2222.options.opts.sd_checkpoint_cache:
...
def load_model(info=None):
    from modules.stable_diffusion_auto1111 import modelsplit, sd_hijack
    info = info or select_checkpoint()

    if info.config != modules.stable_diffusion_auto2222.sdpaths.config:
    ...
    if SDPlugin.lowvram or SDPlugin.medvram:
        lowvram.setup_for_low_vram(sdmodel, SDPlugin.medvram)
    else:
...
def reload_model_weights(sdmodel, info=None):
    ...
    if modules.stable_diffusion_auto2222.sdpaths.config != info.config or should_hijack_inpainting(info) != should_hijack_inpainting(sdmodel.info):
        all_loaded.clear()
        load_model(info)
