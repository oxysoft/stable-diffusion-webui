...
from modules.stable_diffusion_auto1111 import SDPlugin, devices, sd_hijack, sd_models
...
from modules.stable_diffusion_auto1111.image_embedding import (embedding_to_b64, embedding_from_b64,
...
def create_embedding(name, num_vectors_per_token, overwrite_old, init_text='*'):
    ...
    fn = os.path.join(modules.stable_diffusion_auto2222.sdpaths.embeddings_dir, f"{name}.pt")
...
def write_loss(log_directory, filename, step, epoch_len, values):
    if modules.stable_diffusion_auto2222.options.opts.training_write_csv_every == 0:
        return

    if step % modules.stable_diffusion_auto2222.options.opts.training_write_csv_every != 0:
        return
...
def train_embedding(embedding_name, learn_rate, batch_size, data_root, log_directory, training_width, training_height, steps, create_image_every, save_embedding_every, template_file, save_image_with_stored_embedding, preview_from_txt2img, preview_prompt, preview_negative_prompt, preview_steps, preview_sampler_index, preview_cfg_scale, preview_seed, preview_width, preview_height):
    ...
    filename = os.path.join(modules.stable_diffusion_auto2222.sdpaths.embeddings_dir, f'{embedding_name}.pt')
    ...
    with torch.autocast("cuda"):
        ds = modules.textual_inversion.dataset.PersonalizedBase(data_root=data_root, width=training_width, height=training_height, repeats=modules.stable_diffusion_auto2222.options.opts.training_image_repeats_per_epoch, placeholder_token=embedding_name, model=SDPlugin.sdmodel, device=devices.device, template_file=template_file, batch_size=batch_size)
