...
import modules.stable_diffusion_auto1111.options
import modules.stable_diffusion_auto1111.sd_paths
import modules.stable_diffusion_auto1111.sd_textinv_dataset
import torch
import tqdm
from einops import rearrange, repeat
from ldm.util import default
from modules.stable_diffusion_auto1111 import devices, sd_models, SDPlugin
from modules.stable_diffusion_auto1111.sd_textinv_learn_schedule import LearnRateScheduler
...
def init():
    global hypernetworks, loaded_hypernetwork
    hypernetworks = discover_hypernetworks(hypernetwork_dir)
    loaded_hypernetwork = None
...
def apply_strength(value=None):
    HypernetworkModule.multiplier = value if value is not None else modules.stable_diffusion_auto2222.options.opts.sd_hypernetwork_strength
...
def discover_hypernetworks(path):
    res = {}
...
def load_hypernetwork(filename):
    path = SDPlugin.hypernetworks.get(filename, None)
    if path is not None:
        print(f"Loading hypernetwork {filename}")
        try:
            SDPlugin.loaded_hypernetwork = Hypernetwork()
            SDPlugin.loaded_hypernetwork.load(path)

        except Exception:
            print(f"Error loading hypernetwork {path}", file=sys.stderr)
            print(traceback.format_exc(), file=sys.stderr)
    else:
        if SDPlugin.loaded_hypernetwork is not None:
            print(f"Unloading hypernetwork")

        SDPlugin.loaded_hypernetwork = None
...
def find_hypernetwork(search: str, exact=False):
    if not search:
        return None
    search = search.lower()
    applicable = [name for name in SDPlugin.hypernetworks if search in name.lower()]
    if not applicable:
        return None
...
def attention_CrossAttention_forward(self, x, context=None, mask=None):
    ...
    context_k, context_v = apply_hypernetwork(SDPlugin.loaded_hypernetwork, context, self)
...
def train_hypernetwork(hypernetwork_name, learn_rate, batch_size, data_root, log_directory, training_width, training_height, steps, create_image_every, save_hypernetwork_every, template_file, preview_from_txt2img, preview_prompt, preview_negative_prompt, preview_steps, preview_sampler_index, preview_cfg_scale, preview_seed, preview_width, preview_height):
    ...
    from modules.stable_diffusion_auto1111 import images

    assert hypernetwork_name, 'hypernetwork not selected'

    path = SDPlugin.hypernetworks.get(hypernetwork_name, None)
    SDPlugin.loaded_hypernetwork = Hypernetwork()
    SDPlugin.loaded_hypernetwork.load(path)
    ...
    filename = os.path.join(modules.stable_diffusion_auto2222.sdpaths.hypernetwork_dir, f'{hypernetwork_name}.pt')

    log_directory = os.path.join(log_directory, datetime.datetime.now().strftime("%Y-%m-%d"), hypernetwork_name)
    unload = modules.stable_diffusion_auto2222.options.opts.unload_models_when_training
    ...
    with torch.autocast("cuda"):
        ds = modules.textual_inversion.dataset.PersonalizedBase(data_root=data_root, width=training_width, height=training_height, repeats=modules.stable_diffusion_auto2222.options.opts.training_image_repeats_per_epoch, placeholder_token=hypernetwork_name, model=SDPlugin.sdmodel, device=devices.device, template_file=template_file, include_cond=True, batch_size=batch_size)
    if unload:
        SDPlugin.sdmodel.cond_stage_model.to(devices.cpu)
        SDPlugin.sdmodel.first_stage_model.to(devices.cpu)

    hypernetwork = SDPlugin.loaded_hypernetwork
    ...
    for i, entries in pbar:
        ...
        if hypernetwork.step > 0 and images_dir is not None and hypernetwork.step % create_image_every == 0:
            ...
            processed = modules.stable_diffusion_auto2222.processing.process_images(p)
            ...
            if image is not None:
                ...
                last_saved_image, last_text_info = images.save_image(image, images_dir, "", p.seed, p.prompt, modules.stable_diffusion_auto2222.options.opts.samples_format, processed.infotexts[0], p=p, forced_filename=forced_filename)
...
"""
    ...
    filename = os.path.join(modules.stable_diffusion_auto2222.sdpaths.hypernetwork_dir, f'{hypernetwork.name}.pt')
