...
# TODO why is this a new class instead of just a function? can we generalize to BetterDataset or something?
class TextInvDataset(Dataset):
    def __init__(self, data_root, width, height, repeats, flip_p=0.5, placeholder_token="*", model=None, device=None, template_file=None, include_cond=False):
        ...
        for path in tqdm.tqdm(self.image_paths):
            ...
            init_latent = init_latent.to(devicelib.cpu)

            if include_cond:
                text = self.create_text(filename_tokens)
                cond = cond_model([text]).to(devicelib.cpu)
            else:
                cond = None
