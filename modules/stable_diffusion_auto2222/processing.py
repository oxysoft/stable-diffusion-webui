...
def process_images(p: StableDiffusionProcessing):
    ...
    with torch.no_grad(), shared.sd_model.ema_scope():
        with devices.autocast():
            p.init(shared.sd_model, p.all_prompts, p.all_seeds, p.all_subseeds)
        ...
        with devices.autocast():
            uc = prompt_parser.get_learned_conditioning(shared.sd_model, len(prompts) * [p.promptneg], p.steps)
...
# class Processed:
#     def __init__(self, p: SDJob, images_list, seed=-1, info="", subseed=None, all_prompts=None, all_seeds=None, all_subseeds=None, index_of_first_image=0, infotexts=None):
